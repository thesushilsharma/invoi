<script lang="ts">
	...
	const selectedIntervalLabel = $derived(
		recurringIntervals.find((interval) => interval.value === formData.recurringInterval)?.label ||
			'Select interval'
	);
			...
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
				<div>
					...
					{#if errors.invoiceNumber}
						<p class="mt-1 text-sm text-red-500">{errors.invoiceNumber}</p>
					{/if}
				</div>
				...
				<div>
					...
					/>
					{#if errors.taxRate}
						<p class="mt-1 text-sm text-red-500">{errors.taxRate}</p>
					{/if}
				</div>
				...
				<div>
					...
					/>
					{#if errors.issueDate}
						<p class="mt-1 text-sm text-red-500">{errors.issueDate}</p>
					{/if}
				</div>
				...
				<div>
					...
					/>
					{#if errors.dueDate}
						<p class="mt-1 text-sm text-red-500">{errors.dueDate}</p>
					{/if}
				</div>
			</div>
		</CardContent>
		...
		<CardContent class="space-y-4">
			<div>
				...
				{#if errors.clientName}
					<p class="mt-1 text-sm text-red-500">{errors.clientName}</p>
				{/if}
			</div>
			...
			<div>
				...
				{#if errors.clientEmail}
					<p class="mt-1 text-sm text-red-500">{errors.clientEmail}</p>
				{/if}
			</div>
			...
			<div>
				...
				{#if errors.clientAddress}
					<p class="mt-1 text-sm text-red-500">{errors.clientAddress}</p>
				{/if}
			</div>
		</CardContent>
	</Card>
				...
				{#each formData.items as item, index}
					<div class="grid grid-cols-1 gap-4 rounded-lg border p-4 md:grid-cols-5">
						...
						<div class="flex items-end gap-2">
							<div class="flex-1">
								<Label>Total</Label>
								<div class="h-10 rounded-md border bg-muted px-3 py-2">
	...
	<Card>
		...
		<CardContent class="space-y-4">
			...
			<div class="flex items-center space-x-2">
				<Checkbox id="isRecurring" bind:checked={formData.isRecurring} />
